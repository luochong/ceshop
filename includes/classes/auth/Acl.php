...
/**
 * @see auth_Acl_Role_Registry
 */
require_once 'auth/Acl/Role/Registry.php';


/**
 * @see auth_Acl_Assert_Interface
 */
require_once 'auth/Acl/Assert/Interface.php';
...
class auth_Acl
{
    ...
    /**
     * Resource Operate index
     * 
     */ 
    protected  $_resources_operate = array();
     ...
     * @param  auth_Acl_Role_Interface|string|array $parents
     * @uses   auth_Acl_Role_Registry::add()
     * @return auth_Acl Provides a fluent interface
     */
    public function addRole(auth_Acl_Role_Interface $role, $parents = null)
    {
     ...
     *
     * @param  auth_Acl_Role_Interface|string $role
     * @uses   auth_Acl_Role_Registry::get()
     * @return auth_Acl_Role_Interface
     */
     ...
     *
     * @param  auth_Acl_Role_Interface|string $role
     * @uses   auth_Acl_Role_Registry::has()
     * @return boolean
     */
     ...
     *
     * @param  auth_Acl_Role_Interface|string $role
     * @param  auth_Acl_Role_Interface|string $inherit
     * @param  boolean                        $onlyParents
     * @uses   auth_Acl_Role_Registry::inherits()
     * @return boolean
     */
     ...
     *
     * @param  auth_Acl_Role_Interface|string $role
     * @uses   auth_Acl_Role_Registry::remove()
     * @return auth_Acl Provides a fluent interface
     */
    public function removeRole($role)
    {
        ...
        if ($role instanceof auth_Acl_Role_Interface) {
     ...
     *
     * @uses   auth_Acl_Role_Registry::removeAll()
     * @return auth_Acl Provides a fluent interface
     */
     ...
     * @param  Array $operate
     * @param  auth_Acl_Resource_Interface|string $parent
     * @throws auth_Acl_Exception
     * @return auth_Acl Provides a fluent interface
     */
    public function add(auth_Acl_Resource_Interface $resource, $operate,$parent = null)
    {
    	
        $resourceId = $resource->getResourceId();

        $this->_setResourceOperate($resourceId,$operate);
        if ($this->has($resourceId)) {
            require_once 'auth/Acl/Exception.php';
            throw new auth_Acl_Exception("Resource id '$resourceId' already exists in the ACL");
        }
        ...
        if (null !== $parent) {
            try {
                if ($parent instanceof auth_Acl_Resource_Interface) {
                    $resourceParentId = $parent->getResourceId();
                } else {
                    $resourceParentId = $parent;
                }
                $resourceParent = $this->get($resourceParentId);
            } catch (auth_Acl_Exception $e) {
                throw new auth_Acl_Exception("Parent Resource id '$resourceParentId' does not exist");
            }
     ...
     *
     * @param  auth_Acl_Resource_Interface|string $resource
     * @throws auth_Acl_Exception
     * @return auth_Acl_Resource_Interface
     */
    public function get($resource)
    {
        if ($resource instanceof auth_Acl_Resource_Interface) {
            $resourceId = $resource->getResourceId();
        } else {
        ...
        if (!$this->has($resource)) {
            require_once 'auth/Acl/Exception.php';
            throw new auth_Acl_Exception("Resource '$resourceId' not found");
        }
     ...
     *
     * @param  auth_Acl_Resource_Interface|string $resource
     * @param  auth_Acl_Resource_Interface|string $inherit
     * @param  boolean                            $onlyParent
     * @throws auth_Acl_Resource_Registry_Exception
     * @return boolean
     */
     ...
     *
     * @param  auth_Acl_Role_Interface|string|array     $roles
     * @param  auth_Acl_Resource_Interface|string|array $resources
     * @param  string|array                             $privileges
     * @param  auth_Acl_Assert_Interface                $assert
     * @uses   auth_Acl::setRule()
     * @return auth_Acl Provides a fluent interface
     */
    public function allow($roles = null, $resources = null, $privileges = null, auth_Acl_Assert_Interface $assert = null)
    {
        if($this->haveOperate($resources,$privileges)){
    	
    		return $this->setRule(self::OP_ADD, self::TYPE_ALLOW, $roles, $resources, $privileges, $assert);
        }else{
        	require_once 'auth/Acl/Exception.php';
            throw new auth_Acl_Exception("ResourceOperate id '$privileges' not exists in the ACL");
        }
    }
     ...
     * @return auth_Acl Provides a fluent interface
     */
    public function deny($roles = null, $resources = null, $privileges = null, auth_Acl_Assert_Interface $assert = null)
    {
    ...
    public function setRule($operation, $type, $roles = null, $resources = null, $privileges = null,
                            auth_Acl_Assert_Interface $assert = null)
    {
        // ensure that the rule type is valid; normalize input to uppercase
        $type = strtoupper($type);
        if (self::TYPE_ALLOW !== $type && self::TYPE_DENY !== $type) {
            require_once 'auth/Acl/Exception.php';
            throw new auth_Acl_Exception("Unsupported rule type; must be either '" . self::TYPE_ALLOW . "' or '"
                                       . self::TYPE_DENY . "'");
        }
        ...
        switch ($operation) {
            ...
            default:
                require_once 'auth/Acl/Exception.php';
                throw new auth_Acl_Exception("Unsupported operation; must be either '" . self::OP_ADD . "' or '"
                                           . self::OP_REMOVE . "'");
        }
     ...
     * (allow everything to all). By default, auth_Acl creates a "whitelist" rule (deny
     ...
     *
     * @return auth_Acl_Role_Registry
     */
    protected function _getRoleRegistry()
    {
        if (null === $this->_roleRegistry) {
            $this->_roleRegistry = new auth_Acl_Role_Registry();
        }
     ...
     * @return boolean|null
     */
    protected function _roleDFSAllPrivileges(auth_Acl_Role_Interface $role, auth_Acl_Resource_Interface $resource = null)
    {
        $dfs = array(
            'visited' => array(),
            'stack'   => array()
     ...
     * @throws auth_Acl_Exception
     */
    protected function _roleDFSVisitAllPrivileges(auth_Acl_Role_Interface $role, auth_Acl_Resource_Interface $resource = null,
                                                 &$dfs = null)
    {
        if (null === $dfs) {
            /**
             * @see auth_Acl_Exception
             */
            require_once 'auth/Acl/Exception.php';
            throw new auth_Acl_Exception('$dfs parameter may not be null');
        }
     ...
     * @throws auth_Acl_Exception
     */
    protected function _roleDFSOnePrivilege(auth_Acl_Role_Interface $role, auth_Acl_Resource_Interface $resource = null,
                                            $privilege = null)
    {
        if (null === $privilege) {
            /**
             ...
             */
            require_once 'auth/Acl/Exception.php';
            throw new auth_Acl_Exception('$privilege parameter may not be null');
        }
     ...
     * @throws auth_Acl_Exception
     */
    protected function _roleDFSVisitOnePrivilege(auth_Acl_Role_Interface $role, auth_Acl_Resource_Interface $resource = null,
     ...
     * @return string|null
     */
    protected function _getRuleType(auth_Acl_Resource_Interface $resource = null, auth_Acl_Role_Interface $role = null,
                                    $privilege = null)
    {
     ...
     * @return array|null
     */
    protected function &_getRules(auth_Acl_Resource_Interface $resource = null, auth_Acl_Role_Interface $role = null,
                                  $create = false)
    {
    ...
    protected  function _setResourceOperate($resource,$operate){
    	
    	if(!is_array($operate)) $operate = array($operate);
    	$this->_resources_operate[$resource] = $operate;  	
    	
    }
    
    public function getResourceOperate($resource){    	
    	
    	return $this->_resources_operate[$resource];
    
    }
    public function haveOperate($resources,$privileges){
    	return in_array($privileges,$this->getResourceOperate($resources));
    }
