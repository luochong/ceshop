...
class auth_Acl_Role_Registry
{
     ...
     * @throws auth_Acl_Role_Registry_Exception
     * @return auth_Acl_Role_Registry Provides a fluent interface
     */
    public function add(auth_Acl_Role_Interface $role, $parents = null)
    {
        $roleId = $role->getRoleId();

        if ($this->has($roleId)) {
            /**
             * @see auth_Acl_Role_Registry_Exception
             */
            require_once 'auth/Acl/Role/Registry/Exception.php';
            throw new auth_Acl_Role_Registry_Exception("Role id '$roleId' already exists in the registry");
        }
        ...
        if (null !== $parents) {
            ...
            foreach ($parents as $parent) {
                try {
                    if ($parent instanceof auth_Acl_Role_Interface) {
                        $roleParentId = $parent->getRoleId();
                    } else {
                        $roleParentId = $parent;
                    }
                    $roleParent = $this->get($roleParentId);
                } catch (auth_Acl_Role_Registry_Exception $e) {
                    throw new auth_Acl_Role_Registry_Exception("Parent Role id '$roleParentId' does not exist");
                }
    ...
    public function get($role)
    {
        ...
        if (!$this->has($role)) {
             ...
             */
            require_once 'auth/Acl/Role/Registry/Exception.php';
            throw new auth_Acl_Role_Registry_Exception("Role '$roleId' not found");
        }
