...
function display_repeat_div(){
	var repeatId=document.getElementById("repeat_div");
	repeatId.setAttribute("style","display:block");
	repeatId.style.cssText="display:block";
 ...
 var parts;
  ...
  return true;
	 ...
	 function check_date(id){
	 	var date=id.value;
	 	if(!isDatePart(date)){
	 		alert('您输入的日期格式不合法！应类似为：2008-01-01');
	 	}
	 }
	 function check_end_date(start_id,end_id){ 
	 	var start_date=getObject(start_id).value;
	    var end_date=getObject(end_id).value;
